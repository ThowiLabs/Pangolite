...
- Directorio global de Conexiones SSH para abrir la terminal del servidor Pangolite o de cualquier cliente sin navegar primero por su proyecto, con accesos **Más usados** calculados por usuario según su historial real de conexiones.
...
Pangolite también recuerda por usuario el último directorio real de cada destino. Mientras la terminal está abierta, el navegador solicita periódicamente el `cwd` al proceso dueño del PTY y vuelve a comprobarlo después de ejecutar comandos. Si la conexión se corta, **Reconectar** crea la nueva PTY en el último directorio confirmado. Al volver a entrar a `/terminal`, si existe una ruta previa, se muestra la ruta exacta y se pregunta si se desea **Reanudar** allí o iniciar desde la carpeta predeterminada. El servidor local valida la ruta al iniciar; para clientes remotos la capacidad se negocia como `terminal-cwd-v1`, de modo que un servidor nuevo no envía controles de directorio a agentes antiguos.

Para descargar desde la consola se usa `download ruta`. No se instala ningún ejecutable `download` en el servidor o cliente: el navegador intercepta esa línea antes de que el shell la ejecute, Pangolite resuelve la ruta contra el directorio actual del PTY y entrega un ticket HTTP autenticado, temporal y de un solo uso. Los archivos regulares se transmiten directamente al navegador sin cargarlos completos en RAM. Si la ruta es un directorio, se genera un ZIP en streaming con `archive/zip`: no se siguen symlinks ni se incluyen sockets/FIFOs/dispositivos, se bloquean raíces/directorios sensibles del sistema (incluido `/var` completo, permitiendo rutas de datos concretas como `/var/www`) y se limita el árbol a 20.000 entradas y 8 GiB sin comprimir. Los formatos ya comprimidos y los archivos de 128 MiB o más se almacenan sin recomprimir; los archivos menores potencialmente compresibles usan Deflate de velocidad alta. Además, solo puede existir un ZIP activo por proceso y se permiten como máximo dos descargas de terminal simultáneas, evitando que un VPS pequeño quede monopolizado por compresión concurrente.

En terminales remotas las funciones se negocian mediante `terminal-upload-v1`, `terminal-download-v1` y `terminal-cwd-v1`. Un servidor nuevo no envía protocolos de archivo ni controles de directorio no soportados a clientes antiguos: para transferencias muestra un error solicitando actualizar `pangolite-client` y para reanudación conserva el arranque tradicional, evitando que controles o bytes reservados terminen accidentalmente como entrada del shell.
