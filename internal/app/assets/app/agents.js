...
async function toggleAgentWebMaintenance(id,suspend,button=null){const a=agents.find(x=>x.id===id);const name=a?a.name:shortID(id);if(suspend){const webCount=a?(a.webEnabledCount||a.webResourceCount||0):0;if(webCount<=0&&!await confirmAction('Suspender web del cliente','No se detectan recursos web activos para '+name+'. La acción no afecta túneles TCP/UDP. ¿Quieres continuar?','Continuar',false))return;if(webCount>0&&!await confirmAction('Suspender web del cliente','Se suspenderán los recursos HTTP/HTTPS activos de '+name+'. Los túneles TCP/UDP seguirán funcionando para SSH u otros accesos de mantenimiento.','Suspender web',false))return}else{if(!await confirmAction('Reactivar web del cliente','Se reactivarán solo los recursos web que Pangolite suspendió para mantenimiento en '+name+'. Los recursos que ya estaban apagados antes no se fuerzan a activar.','Reactivar web',false))return}await withActionLoading(button,suspend?'Suspendiendo':'Reactivando',async()=>{try{showBusy(suspend?'Suspendiendo web':'Reactivando web',suspend?'Aplicando mantenimiento web y actualizando Traefik':'Restaurando recursos web suspendidos y actualizando Traefik');const data=await api('/api/agents/'+id+'/web-maintenance',{method:'POST',body:JSON.stringify({suspended:!!suspend})});msg(suspend?'Web del cliente suspendida. TCP/UDP sigue activo.':'Web del cliente reactivada.');await reloadProjects();if(currentProject)await loadProjectData(currentProject.id);return data}catch(err){msg(err.message,true)}finally{hideBusy()}})}
...
async function quickSuspend(id,mode,button=null){const r=resources.find(x=>x.id===id);if(!await confirmAction('Suspender recurso','El recurso '+(r?r.name:shortID(id))+' dejara de responder hacia su backend y mostrara la respuesta configurada.','Suspender',false))return;await withActionLoading(button,'Suspendiendo',async()=>{try{await api('/api/resources/'+id,{method:'PATCH',body:JSON.stringify({enabled:false,disabledResponseMode:mode,disabledStatusCode:mode==='404'?404:403,disabledHtml:''})});msg('Recurso suspendido');await reloadProjects();await loadProjectData(currentProject.id)}catch(err){msg(err.message,true)}})}
async function quickActivate(id,button=null){const r=resources.find(x=>x.id===id);if(!await confirmAction('Activar recurso','El recurso '+(r?r.name:shortID(id))+' volvera a enviar trafico al servicio interno.','Activar',false))return;await withActionLoading(button,'Activando',async()=>{try{await api('/api/resources/'+id,{method:'PATCH',body:JSON.stringify({enabled:true,disabledResponseMode:(r&&r.disabledResponseMode)||'403',disabledStatusCode:(r&&r.disabledStatusCode)||403,disabledHtml:(r&&r.disabledHtml)||''})});msg('Recurso activado');await reloadProjects();await loadProjectData(currentProject.id)}catch(err){msg(err.message,true)}})}
...
async function deleteCurrentProject(button=null){if(!currentProject)return;const st=stats[currentProject.id]||{};if((st.resources||resources.length)>0||(st.agents||agents.length)>0){msg('Primero elimina todos los recursos y clientes de sistema de este proyecto',true);return}if(!await confirmAction('Eliminar proyecto','Se eliminara el proyecto '+currentProject.name+'. Esta accion no se puede deshacer.','Eliminar proyecto'))return;await withActionLoading(button,'Eliminando',async()=>{try{await api('/api/projects/'+currentProject.id,{method:'DELETE'});msg('Proyecto eliminado');currentProject=null;await reloadProjects();go('/projects')}catch(err){msg(err.message,true)}})}
async function deleteAgent(id,button=null){const a=agents.find(x=>x.id===id);const count=a?(a.resourceCount||0):0;const body='Eliminar el cliente de sistema '+(a?a.name:shortID(id))+' también eliminará '+count+' recurso(s) vinculado(s). Escribe tu contraseña para confirmar.';const password=await confirmPasswordAction('Eliminar cliente de sistema',body,'Eliminar cliente de sistema y recursos');if(!password)return;await withActionLoading(button,'Eliminando',async()=>{try{showBusy('Eliminando cliente de sistema','Eliminando recursos vinculados y aplicando Traefik');const res=await api('/api/agents/'+id,{method:'DELETE',body:JSON.stringify({password})});msg('Cliente de sistema eliminado. Recursos eliminados: '+(res.deletedResources||0));await reloadProjects();if(currentProject)await loadProjectData(currentProject.id)}catch(err){msg(err.message,true)}finally{hideBusy()}})}
async function rotateAgentToken(id,button=null){const a0=agents.find(x=>x.id===id);if(!await confirmAction('Rotar token del cliente de sistema','El token actual dejara de servir. Deberas reinstalar o actualizar el cliente de sistema '+(a0?a0.name:shortID(id))+' con el nuevo comando. Tambien se incluirá fallback por IP si el servidor lo conoce.','Rotar token'))return;await withActionLoading(button,'Rotando',async()=>{try{const a=await api('/api/agents/'+id+'/token',{method:'POST',body:'{}'});const os=selectedAgentOS(a0,'linux');$('agentTokenBox').replaceChildren(renderAgentCredentials(a,os));$('agentTokenBox').classList.remove('d-none');showAgentCredentialsDialog(a,{title:'Token rotado',meta:'El token anterior ya no conecta. Usa Instalar/actualizar para sobrescribir la configuracion o Eliminar para borrar una instalacion vieja. El fallback por IP queda incluido si esta disponible.',os});msg('Token rotado. Copia el nuevo comando ahora.');if(currentProject)await loadProjectData(currentProject.id)}catch(err){msg(err.message,true)}})}
...
async function markDomainLegacy(id,button=null){const d=domains.find(x=>x.id===id);if(!await confirmAction('Marcar dominio heredado','El dominio '+(d?d.domain:shortID(id))+' dejará de aparecer en nuevas instalaciones, pero se conservará para clientes antiguos.','Marcar heredado',false))return;await withActionLoading(button,'Marcando',async()=>{try{const data=await api('/api/domains/'+id,{method:'PATCH',body:JSON.stringify({status:'legacy'})});await refreshDomainsFromResponse(data);msg('Dominio marcado como heredado')}catch(err){msg(err.message,true)}})}
async function activateDomain(id,button=null){await withActionLoading(button,'Activando',async()=>{try{const data=await api('/api/domains/'+id,{method:'PATCH',body:JSON.stringify({status:'active'})});await refreshDomainsFromResponse(data);msg('Dominio activado para nuevas configuraciones')}catch(err){msg(err.message,true)}})}
async function makeDomainPrimary(id,button=null){const d=domains.find(x=>x.id===id);const name=d?d.domain:shortID(id);if(!await confirmAction('Cambiar dominio principal','El dominio '+name+' será el dominio del panel y de los nuevos clientes. El dominio principal anterior pasará a Heredado si tiene uso. No se elimina nada en este paso.','Hacer principal',false))return;await withActionLoading(button,'Validando',async()=>{try{const data=await api('/api/domains/'+id,{method:'PATCH',body:JSON.stringify({primary:true})});await refreshDomainsFromResponse(data);msg('Dominio principal actualizado. Los clientes nuevos usarán '+name)}catch(err){msg(err.message,true)}})}
async function deleteDomain(id,button=null){const d=domains.find(x=>x.id===id);if(d&&d.deleteLocked){msg(d.deleteReason||'No se puede eliminar este dominio todavía.',true);return}if(d&&d.primary){msg('No se puede eliminar el dominio principal. Primero establece otro dominio como principal.',true);return}if(d&&(d.resourceCount||0)>0){msg('No se puede eliminar: el dominio todavía tiene recursos asociados.',true);return}const name=d?d.domain:shortID(id);let body='Se eliminará definitivamente '+name+'.';if(d&&(d.agentCount||0)>0){body+=' Hay '+(d.agentCount||0)+' cliente(s) vinculados, pero el servidor los considera seguros porque ya confirmaron fallback por IP. Si esos clientes fueron reinstalados manualmente sin fallback, no elimines este dominio todavía.'}else{body+=' Esta acción solo debería usarse cuando ya no tiene clientes ni recursos asociados.'}if(!await confirmAction('Eliminar dominio administrado',body,'Eliminar dominio'))return;await withActionLoading(button,'Eliminando',async()=>{try{const data=await api('/api/domains/'+id,{method:'DELETE'});await refreshDomainsFromResponse(data);msg('Dominio eliminado')}catch(err){msg(err.message,true)}})}
async function renderTraefik(button=null){await withActionLoading(button,'Aplicando',async()=>{try{const r=await api('/api/render-traefik',{method:'POST',body:'{}'});msg(r.message||'Traefik actualizado')}catch(err){msg(err.message,true)}})}
