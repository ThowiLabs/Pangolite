...
function paintProjectTable(){const rows=maybeEl('projectRows');if(!rows)return;clearNode(rows);if(!projects.length){rows.appendChild(makeEmpty('No hay proyectos. Usa + Crear para iniciar el onboarding y agregar el primer proyecto.'));return}projects.forEach(p=>{const st=stats[p.id]||{};const card=tplNode('tpl-project-card');card.classList.toggle('project-card-disabled',!p.enabled);appendSlot(card,'state',stateNode(!!p.enabled,'Activo','Inactivo'));setSlot(card,'name',p.name);setSlot(card,'slug','/'+p.slug);setSlot(card,'resources',st.resources||0);setSlot(card,'agents',st.agents||0);setSlot(card,'active',st.activeResources||0);const open=slot(card,'open');if(open){open.href='/projects/'+p.id;open.setAttribute('aria-label','Abrir '+p.name)}const actions=slot(card,'actions');const summary=makeLink('Resumen','/projects/'+p.id,'bi-command','btn btn-sm btn-primary pl-icon-action');summary.title='Resumen';summary.setAttribute('aria-label','Abrir resumen de '+p.name);const resourceLink=makeLink('Recursos','/projects/'+p.id+'/resources','bi-diagram-3','btn btn-sm btn-outline-secondary pl-icon-action');resourceLink.title='Recursos';resourceLink.setAttribute('aria-label','Ver recursos de '+p.name);const agentLink=makeLink('Clientes','/projects/'+p.id+'/agents','bi-hdd-network','btn btn-sm btn-outline-secondary pl-icon-action');agentLink.title='Clientes';agentLink.setAttribute('aria-label','Ver clientes de '+p.name);[summary,resourceLink,agentLink].forEach(link=>{const text=link.querySelector('span');if(text)text.classList.add('visually-hidden');actions.appendChild(link)});rows.appendChild(card)})}
...
function fillDomainSelect(){const sel=$('domainSelect');if(!sel)return;const current=sel.value;clearNode(sel);domains.filter(d=>d.enabled&&d.status!=='legacy').forEach(d=>{const opt=document.createElement('option');opt.value=d.domain;opt.textContent=d.primary?d.domain+' (principal)':d.domain;sel.appendChild(opt)});const custom=document.createElement('option');custom.value='custom';custom.textContent='Custom';sel.appendChild(custom);if(current&&Array.from(sel.options).some(o=>o.value===current))sel.value=current;else if(!domains.filter(d=>d.enabled&&d.status!=='legacy').length)sel.value='custom';syncDomainMode();refreshWidgetSelect('domainSelect')}
...
async function loadProjectData(id){if(typeof selectedResourceIDs!=='undefined')selectedResourceIDs.clear();if(typeof selectedAgentIDs!=='undefined')selectedAgentIDs.clear();resources=(await api('/api/resources?projectId='+encodeURIComponent(id))).resources||[];agents=(await api('/api/agents?projectId='+encodeURIComponent(id))).agents||[];paintResources();paintAgents();fillAgentSelect();updateSidebarProjectContext();syncHTTPResourceKind();syncEditResourceKind();}
