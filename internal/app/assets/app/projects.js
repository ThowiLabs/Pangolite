...
function paintDomains(){
  ...
  domains.forEach(d=>{
    ...
    if(!d.primary&&d.status!=='legacy')actions.appendChild(domainActionButton('Heredar','bi-archive','btn btn-sm btn-outline-secondary',()=>markDomainLegacy(d.id),'Oculta el dominio en nuevas instalaciones, pero conserva compatibilidad.'));
    if(!d.primary)actions.appendChild(domainActionButton('Hacer principal','bi-star','btn btn-sm btn-outline-secondary',()=>makeDomainPrimary(d.id),'Usar como dominio del panel y nuevos clientes.'));
    if(d.status==='legacy')actions.appendChild(domainActionButton('Activar','bi-toggle-on','btn btn-sm btn-outline-secondary',()=>activateDomain(d.id),'Volver a mostrar como dominio disponible.'));
    ...
    const del=domainActionButton('Eliminar','bi-trash','btn btn-sm btn-outline-danger',()=>deleteDomain(d.id),deleteHint);
    del.disabled=locked;
