...
async function loadSettings(){const data=await api('/api/settings');panelSettings=data.settings||{};networkInfo=data.network||{};if($('dashboardDomain'))$('dashboardDomain').value=panelSettings.dashboardDomain||'';if($('letsEncryptEmail'))$('letsEncryptEmail').value=panelSettings.letsEncryptEmail||'';if($('backupIntervalHours'))$('backupIntervalHours').value=panelSettings.backupIntervalHours??24;if($('backupRetentionDays'))$('backupRetentionDays').value=panelSettings.backupRetentionDays??14;if($('smtpEnabled'))$('smtpEnabled').checked=!!panelSettings.smtpEnabled;if($('smtpHost'))$('smtpHost').value=panelSettings.smtpHost||'';if($('smtpPort'))$('smtpPort').value=panelSettings.smtpPort||587;if($('smtpSecurity'))$('smtpSecurity').value=panelSettings.smtpSecurity||'starttls';if($('smtpUsername'))$('smtpUsername').value=panelSettings.smtpUsername||'';if($('smtpPassword'))$('smtpPassword').value='';if($('smtpFromEmail'))$('smtpFromEmail').value=panelSettings.smtpFromEmail||'';if($('smtpFromName'))$('smtpFromName').value=panelSettings.smtpFromName||'';paintNetworkInfo(data.certificate)}
...
function currentSettingsPayload(){return {dashboardDomain:($('dashboardDomain')?$('dashboardDomain').value.trim().toLowerCase():(panelSettings.dashboardDomain||'')),letsEncryptEmail:($('letsEncryptEmail')?$('letsEncryptEmail').value.trim().toLowerCase():(panelSettings.letsEncryptEmail||'')),backupIntervalHours:$('backupIntervalHours')?fieldNumber('backupIntervalHours'):(panelSettings.backupIntervalHours??24),backupRetentionDays:$('backupRetentionDays')?fieldNumber('backupRetentionDays'):(panelSettings.backupRetentionDays??14),smtpEnabled:!!($('smtpEnabled')&&$('smtpEnabled').checked),smtpHost:($('smtpHost')?$('smtpHost').value.trim():(panelSettings.smtpHost||'')),smtpPort:$('smtpPort')?fieldNumber('smtpPort'):(panelSettings.smtpPort||587),smtpSecurity:($('smtpSecurity')?$('smtpSecurity').value:(panelSettings.smtpSecurity||'starttls')),smtpUsername:($('smtpUsername')?$('smtpUsername').value.trim():(panelSettings.smtpUsername||'')),smtpPassword:($('smtpPassword')?$('smtpPassword').value:''),smtpFromEmail:($('smtpFromEmail')?$('smtpFromEmail').value.trim().toLowerCase():(panelSettings.smtpFromEmail||'')),smtpFromName:($('smtpFromName')?$('smtpFromName').value.trim():(panelSettings.smtpFromName||''))}}
async function saveSettings(e){e.preventDefault();try{const data=await api('/api/settings',{method:'PATCH',body:JSON.stringify(currentSettingsPayload())});panelSettings=data.settings||{};networkInfo=data.network||{};if($('smtpPassword'))$('smtpPassword').value='';paintNetworkInfo(data.certificate);await reloadDomains();const t=data.traefik||{};const cert=data.certificate?(' SSL: '+certText(data.certificate)+'.'):'';const source=e&&e.target&&e.target.id==='smtpSettingsForm'?'SMTP guardado y validado. ':'Ajustes guardados. ';msg(source+(t.message||'Traefik se actualizo automaticamente.')+cert)}catch(err){msg(err.message,true)}}
function smtpProbePayload(){const payload=currentSettingsPayload();payload.smtpEnabled=true;return payload}
async function verifySMTPSettings(button){await withActionLoading(button,'Probando',async()=>{try{const data=await api('/api/settings/smtp/test',{method:'POST',body:JSON.stringify(smtpProbePayload())});msg(data.message||'Conexion SMTP verificada correctamente.',false)}catch(err){msg(err.message||String(err),true)}})}
