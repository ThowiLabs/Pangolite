...
function closePasswordConfirm(){$('passwordConfirmModal').classList.remove('open');setIfExists('passwordConfirmInput','')}
function confirmPasswordAction(title,body,confirmText='Confirmar eliminación'){return new Promise(resolve=>{const modal=$('passwordConfirmModal');$('passwordConfirmTitle').textContent=title;$('passwordConfirmBody').textContent=body;$('passwordConfirmAcceptBtn').innerHTML='<i class="bi bi-trash3"></i> '+esc(confirmText);setIfExists('passwordConfirmInput','');const finish=v=>{modal.classList.remove('open');const pass=fieldValue('passwordConfirmInput');setIfExists('passwordConfirmInput','');resolve(v?pass:null)};$('passwordConfirmCancelBtn').onclick=()=>finish(false);$('passwordConfirmAcceptBtn').onclick=()=>finish(true);$('passwordConfirmInput').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();finish(true)}};modal.classList.add('open');setTimeout(()=>$('passwordConfirmInput').focus(),50)})}
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeHelp();closeProjectModal();closeDomainModal();closeAgentDetail();closeResourceEditModal();closePasswordConfirm()}});['helpModal','projectModal','domainModal','resourceEditModal','agentDetailModal','passwordConfirmModal'].forEach(id=>$(id).addEventListener('click',e=>{if(e.target===$(id))$(id).classList.remove('open')}));
