...
<script>
let csrf='';let projects=[];let stats={};let resources=[];let agents=[];let domains=[];let panelSettings={};let networkInfo={};let currentProject=null;let charts={};let logsLines=[];let resourceHealth={};let deletingResources=new Set();
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeHelp();closeProjectModal();closeDomainModal();closeAgentDetail();closeResourceEditModal()}});['helpModal','projectModal','domainModal','resourceEditModal','agentDetailModal'].forEach(id=>$(id).addEventListener('click',e=>{if(e.target===$(id))$(id).classList.remove('open')}));
...
async function disableAgent(id){const a=agents.find(x=>x.id===id);if(!await confirmAction('Deshabilitar cliente de sistema','El cliente '+(a?a.name:shortID(id))+' dejara de aceptar nuevos tuneles hasta que lo reactives o generes credenciales nuevas.','Deshabilitar'))return;try{await api('/api/agents/'+id,{method:'DELETE'});msg('Cliente deshabilitado');await reloadProjects();await loadProjectData(currentProject.id)}catch(err){msg(err.message,true)}}async function rotateAgentToken(id){const a0=agents.find(x=>x.id===id);if(!await confirmAction('Rotar token del cliente','El token actual dejara de servir. Deberas reinstalar o actualizar el cliente '+(a0?a0.name:shortID(id))+' con el nuevo comando.','Rotar token'))return;try{const a=await api('/api/agents/'+id+'/token',{method:'POST',body:'{}'});$('agentTokenBox').innerHTML=renderAgentCredentials(a);$('agentTokenBox').classList.remove('d-none');msg('Token rotado. Copia el nuevo comando ahora.');await loadProjectData(currentProject.id)}catch(err){msg(err.message,true)}}
