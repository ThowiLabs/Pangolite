...
<head>
  ...
  <link rel="icon" href="/assets/app/favicon.ico" sizes="any">
  <link rel="apple-touch-icon" href="/assets/app/logo-mark.png">
...
<body class="auth-shell">
  <main class="auth-wrap">
    <div class="card auth-card w-100">
      <div class="card-body p-4 p-sm-5">
        <div class="d-flex align-items-center gap-3 mb-4">
          <img class="brand-logo" src="/assets/app/logo-mark.png" alt="Pangolite">
          <div>
    ...
    <div class="auth-footer">Creado con amor por <a href="https://thowilabs.com/opensource/" target="_blank" rel="noopener noreferrer">ThowiLabs</a>.</div>
  </main>
<script>
...
<div class="app">
  <aside class="sidebar" aria-label="Navegacion principal">
    ...
    <nav class="nav-section project-primary">
      ...
      <div class="main-layout-note mt-2 px-2">Selecciona primero un proyecto. Todo lo demás depende del proyecto activo.</div>
    </nav>
  ...
  <main class="main">
    ...
    <section id="view-projects" class="route-view">
      ...
      <div id="onboardingCard" class="card onboarding-card mb-3 d-none">
      ...
      <div class="grid grid-3 mb-3 dashboard-panel">
        <div class="stat metric-card soft-enter"><div class="d-flex align-items-center justify-content-between"><div><div class="stat-label">Proyectos</div><div class="stat-value" id="statProjects">0</div><div class="stat-trend">Proyectos registrados</div></div><div class="metric-icon"><i class="bi bi-building"></i></div></div></div>
    ...
    <section id="view-project" class="route-view">
      <div class="hero"><h1 id="projectHeroTitle">Proyecto</h1><p id="projectHeroText">Recursos y agentes separados por proyecto.</p></div>
      ...
      <div class="grid grid-2 mb-3">
        <div class="card"><div class="card-header"><div class="card-title">Configuración del proyecto</div><div class="text-muted small">Renombra el proyecto y actualiza su descripción operativa.</div></div><div class="card-body"><form id="projectSettingsForm"><label class="form-label" for="projectEditName">Nombre</label><input id="projectEditName" class="form-control mb-3" required><label class="form-label" for="projectEditNotes">Descripción</label><textarea id="projectEditNotes" class="form-control mb-3" rows="4" placeholder="Descripción, responsable y notas de operación del proyecto"></textarea><button class="btn btn-primary" type="submit"><i class="bi bi-save"></i> Guardar proyecto</button></form></div></div>
        <div class="card danger-zone"><div class="card-header"><div class="card-title">Zona de peligro</div><div class="text-muted small">Acciones irreversibles del proyecto seleccionado.</div></div><div class="card-body"><p class="danger-note" id="projectDangerHint">Para eliminar el proyecto primero debe quedar sin recursos ni clientes de sistema.</p><button id="deleteProjectBtn" class="btn btn-outline-danger" type="button" onclick="deleteCurrentProject()"><i class="bi bi-trash3"></i> Eliminar proyecto</button></div></div>
      </div>
    ...
    <section id="view-agents" class="route-view">
      ...
      <div class="card"><div class="card-header d-flex justify-content-between align-items-center"><div><div class="card-title">Clientes de sistema</div><div class="text-muted small">ID y token para servidores remotos. Los tokens se muestran solo al crear o rotar.</div></div><div class="d-flex gap-2"><button class="btn btn-help" onclick="showHelp('Clientes de sistema','Un cliente de sistema no es un proxy. Es la credencial instalada en un servidor NAT/remoto. Los recursos despues eligen si usan este cliente o si apuntan directo al host Pangolite.')"><i class="bi bi-question"></i></button><a id="goCreateAgentFromList" class="btn btn-primary" href="#"><i class="bi bi-plus-lg"></i> Crear cliente de sistema</a></div></div><div class="card-body table-responsive"><table class="table"><thead><tr><th>Estado</th><th>Nombre</th><th>ID</th><th>Sistema</th><th>Recursos</th><th>Ultima vez</th><th class="text-end">Acciones</th></tr></thead><tbody id="agentsRows"></tbody></table><div id="agentTokenBox" class="pl-callout pl-primary token-box font-mono mt-3 d-none"></div></div></div>
    </section>

    <section id="view-create-agent" class="route-view">
      <div class="hero"><h1>Crear cliente de sistema</h1><p>Genera el ID y token que se instalarán en un servidor NAT, VPS del proyecto o red remota. Este cliente de sistema no publica nada por sí solo; solo habilita conexión saliente hacia Pangolite.</p></div>
      <div class="grid grid-2">
        <div class="card"><div class="card-header d-flex justify-content-between align-items-center"><div><div class="card-title">Datos del cliente de sistema</div><div class="text-muted small">Identidad de conexion remota.</div></div><button class="btn btn-help" onclick="showHelp('Crear cliente de sistema','Crea un cliente por servidor o red remota. Copia ID y TOKEN al instalar el agente. Despues los recursos podran seleccionar este cliente como origen del backend.')"><i class="bi bi-question"></i></button></div><div class="card-body"><label class="form-label">Nombre del cliente de sistema</label><input id="agentName" class="form-control mb-3" placeholder="servidor-remoto-01"><label class="form-label">Sistema operativo del servidor</label><select id="agentInstallOS" class="form-select mb-2"><option value="linux">Linux / systemd / OpenRC</option><option value="windows">Windows / PowerShell</option></select><div class="form-text mb-3">Elige el sistema donde instalaras el cliente para mostrar solo los comandos correctos.</div><button class="btn btn-primary" onclick="createAgent()"><i class="bi bi-hdd-network"></i> Crear cliente de sistema</button><div id="agentTokenCreate" class="pl-callout pl-primary token-box mt-3 d-none"></div></div></div>
    ...
    <section id="view-create-resource" class="route-view">
      ...
      <div class="card"><div class="card-header d-flex justify-content-between align-items-center"><div><div class="card-title">Datos del recurso</div><div class="text-muted small">Define qué se publica y dónde vive el servicio real.</div></div><button class="btn btn-help" onclick="showHelp('Crear recurso','Primero define que publicas. Luego elige origen: host local si Pangolite puede llegar directo al backend, o cliente de sistema si el backend vive dentro de un VPS/NAT/remoto.')"><i class="bi bi-question"></i></button></div><div class="card-body">
        <form id="resourceForm">
          ...
          <div class="mt-3 http-only d-none" id="customDomainGroup"><label class="form-label">Dominio personalizado</label><input id="customDomain" class="form-control" placeholder="app.proyecto.com"></div>
          <div class="grid grid-2 mt-3 http-only"><div><label class="form-label">Path</label><input id="pathPrefix" class="form-control" value="/"><div class="form-text">Usa / para todo el dominio o /app para una ruta especifica.</div></div><div><label class="form-label">Seguridad</label><div class="form-check form-switch mt-2"><input class="form-check-input" type="checkbox" role="switch" id="tls" checked><label class="form-check-label" for="tls">Usar SSL</label></div><div class="form-text">Activado: HTTPS con ACME. Desactivado: solo HTTP.</div></div></div>
          ...
          <div class="alert pl-surface mt-2 http-only" id="certStatusCreate"><i class="bi bi-shield-lock"></i> SSL se revisará al crear el recurso.</div>
    ...
    <section id="view-logs" class="route-view">
      <div class="hero"><h1>Logs del sistema</h1><p>Ultimos eventos del panel, validaciones de puertos, errores API, Traefik y clientes de sistema. El archivo se mantiene en maximo 1000 entradas para evitar crecimiento indefinido.</p></div>
...
function copyButton(id,label='Copiar'){return '<button class="btn btn-sm btn-outline-secondary copy-btn" type="button" onclick="copyCommand(\''+id+'\',this)" data-copy-label="'+esc(label)+'"><i class="bi bi-clipboard"></i> '+esc(label)+'</button>'}
...
function copyFeedback(btn,ok=true){if(!btn)return;const original=btn.dataset.copyOriginal||btn.innerHTML;btn.dataset.copyOriginal=original;btn.disabled=true;btn.innerHTML=ok?'<i class="bi bi-check2"></i> Copiado':'<i class="bi bi-exclamation-triangle"></i> Error';setTimeout(()=>{btn.innerHTML=btn.dataset.copyOriginal;btn.disabled=false},1300)}
async function copyCommand(id,btn){const value=commandCopies[id]||'';const sx=window.scrollX,sy=window.scrollY;try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(value)}else{const active=document.activeElement;const ta=document.createElement('textarea');ta.value=value;ta.setAttribute('readonly','');ta.style.position='fixed';ta.style.top='0';ta.style.left='0';ta.style.width='1px';ta.style.height='1px';ta.style.opacity='0';ta.style.pointerEvents='none';document.body.appendChild(ta);ta.focus({preventScroll:true});ta.select();document.execCommand('copy');ta.remove();if(active&&active.focus)active.focus({preventScroll:true})}window.scrollTo(sx,sy);copyFeedback(btn,true)}catch(err){window.scrollTo(sx,sy);copyFeedback(btn,false)}}
...
function syncAgentCommandOS(sel){const box=sel.closest('.agent-credentials');if(!box)return;const os=sel.value||'linux';box.querySelectorAll('[data-agent-os]').forEach(el=>el.classList.toggle('d-none',el.getAttribute('data-agent-os')!==os));const note=box.querySelector('[data-agent-os-note]');if(note)note.textContent=os==='windows'?'Ejecuta estos comandos en PowerShell como administrador.':'Ejecuta estos comandos en la shell del servidor Linux con permisos sudo/root.'}
function agentOSSelect(selected){selected=selected==='windows'?'windows':'linux';return '<div class="pl-callout pl-secondary"><label class="form-label">Sistema operativo para comandos</label><select class="form-select agent-os-command-select" onchange="syncAgentCommandOS(this)"><option value="linux" '+(selected==='linux'?'selected':'')+'>Linux / systemd / OpenRC</option><option value="windows" '+(selected==='windows'?'selected':'')+'>Windows / PowerShell</option></select><div class="form-text mt-2" data-agent-os-note>'+(selected==='windows'?'Ejecuta estos comandos en PowerShell como administrador.':'Ejecuta estos comandos en la shell del servidor Linux con permisos sudo/root.')+'</div></div>'}
function agentCommandGroup(os,selected,html){return '<div data-agent-os="'+os+'" class="agent-os-commands '+(selected===os?'':'d-none')+'">'+html+'</div>'}
function renderAgentCredentials(a,preferredOS){const selected=selectedAgentOS(a,preferredOS||'linux');const remove=a.removeCommand||'sudo /opt/pangolite-client/pangolite-client --remove';const wremove=a.windowsRemoveCommand||"Start-Process -Verb RunAs 'C:\\ProgramData\\Pangolite Client\\pangolite-client.exe' -ArgumentList '--remove'";const linux=commandBlock('Instalar Linux systemd/OpenRC',a.installCommand||'')+commandBlock('Eliminar Linux por completo',remove);const windows=commandBlock('Instalar Windows como servicio',a.windowsInstallCommand||'')+commandBlock('Eliminar Windows por completo',wremove);return '<div class="command-stack agent-credentials"><div class="secret-grid">'+secretBlock('ID del cliente de sistema',a.id||'')+secretBlock('Token',a.token||'')+'</div>'+agentOSSelect(selected)+agentCommandGroup('linux',selected,linux)+agentCommandGroup('windows',selected,windows)+'</div>'}
...
async function loadSettings(){const data=await api('/api/settings');panelSettings=data.settings||{};networkInfo=data.network||{};if($('dashboardDomain'))$('dashboardDomain').value=panelSettings.dashboardDomain||'';if($('letsEncryptEmail'))$('letsEncryptEmail').value=panelSettings.letsEncryptEmail||'';paintNetworkInfo(data.certificate)}
...
async function createBackup(){try{const label=await confirmInputAction({title:'Crear respaldo SQLite',body:'Escribe un prefijo opcional para identificar el respaldo. Dejalo vacio para usar el nombre automatico.',label:'Prefijo del respaldo',placeholder:'antes-eliminar-proyecto',confirmText:'Crear respaldo',icon:'bi-database-add',danger:false});if(label===null)return;showBusy('Creando respaldo','SQLite genera una copia consistente sin detener el panel');const backup=await api('/api/backups',{method:'POST',body:JSON.stringify({label})});msg('Respaldo creado: '+backup.name);await loadBackups();await loadAudit()}catch(err){msg(err.message,true)}finally{hideBusy()}}
async function loadMaintenance(){await Promise.all([loadBackups(),loadAudit()])}

function paintProjectNav(filter=''){const box=$('projectNav');box.innerHTML='';const q=String(filter||'').trim().toLowerCase();const list=projects.filter(p=>!q||p.name.toLowerCase().includes(q)||p.slug.toLowerCase().includes(q)||p.id.toLowerCase().includes(q));if(!list.length){box.innerHTML='<div class="empty py-3">Sin proyectos todavía</div>';return}list.forEach(p=>{const st=stats[p.id]||{};const btn=document.createElement('button');btn.type='button';btn.className='project-result'+(currentProject&&currentProject.id===p.id?' active':'');btn.dataset.projectNav=p.id;btn.innerHTML='<span><span class="fw-semibold">'+esc(p.name)+'</span><div class="project-meta">'+esc(p.slug)+' · '+(st.resources||0)+' recursos · '+(st.agents||0)+' agentes</div></span><i class="bi bi-arrow-right-short"></i>';btn.addEventListener('click',()=>{closeProjectSwitcher();go('/projects/'+p.id)});box.appendChild(btn)});updateProjectSwitcherLabel()}
...
function paintProjectTable(){const rows=$('projectRows');rows.innerHTML='';if(!projects.length){rows.innerHTML='<tr><td colspan="7" class="empty">No hay proyectos. Usa + Crear para iniciar el onboarding y agregar el primer proyecto.</td></tr>';return}projects.forEach(p=>{const st=stats[p.id]||{};const tr=document.createElement('tr');tr.innerHTML='<td>'+(p.enabled?'<span class="state-pill on"><span class="status-dot ok"></span>Activo</span>':'<span class="state-pill off"><span class="status-dot"></span>Inactivo</span>')+'</td><td class="fw-semibold text-white">'+esc(p.name)+'</td><td class="font-mono">/'+esc(p.slug)+'</td><td>'+(st.resources||0)+'</td><td>'+(st.agents||0)+'</td><td>'+(st.activeResources||0)+'</td><td class="text-end"><a class="btn btn-sm btn-primary me-1" href="/projects/'+p.id+'"><i class="bi bi-box-arrow-in-right"></i> Abrir</a><a class="btn btn-sm btn-outline-secondary" href="/projects/'+p.id+'"><i class="bi bi-pencil-square"></i> Configurar</a></td>';rows.appendChild(tr)})}
...
async function createAgent(){
  ...
  try{
    ...
    $('agentTokenCreate').innerHTML=renderAgentCredentials(a,fieldValue('agentInstallOS')||'linux');
...
async function createResourceFromForm(e){
  ...
  try{
    ...
    await reloadProjects();
    await loadProjectData(currentProject.id);
    msg('Recurso creado'+(cert?'. SSL: '+certText(cert)+'.':''));
